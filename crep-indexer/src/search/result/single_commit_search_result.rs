...
impl SingleCommitSearchResult {
    pub fn new(
        ...
        let matches_in_file = get_matches_in_file(query, file_content)?;
        if matches_in_file.is_none() {
            return Ok(None);
        }

        let (words_per_line, lines) = matches_in_file.unwrap();
...
> {
    let matches = match query {
        ...
        MatchedQuery::Regex(regex) => {
            ...
            find_regex_matches_in_document(&r, file_content)
        }
    };
